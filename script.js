...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const bioMessages = [
    "Avant que je n'aille me coucher, je prie le Seigneur de garder mon âme, j'espère qu'il n'est pas trop tard pour moi.",
    "\"Hello, World!\""
  ];
  let bioText = '';
  let bioIndex = 0;
  ...
  const leftOffers = document.querySelectorAll('.contact-offer');
  ...
  function closeContact() {
    ...
    gsap.to(contactBlock, {
      ...
      onComplete: () => {
        ...
        profileBlock.classList.remove('hidden');
        gsap.fromTo(profileBlock, { opacity: 0, y: -30 }, { opacity: 1, y: 0, duration: 0.4, ease: 'power2.out' });
      }
    });
  }

  contactBtn.addEventListener('click', () => {
    if (isContactOpen) closeContact(); else openContact();
  });
  ...
  // Code block character scatter animation
  function animateCodeChars() {
    const strings = codeBlock.querySelectorAll('.code-str');
  ...
  codeBlock.addEventListener('click', animateCodeChars);
  codeBlock.addEventListener('touchstart', (e) => { e.preventDefault(); animateCodeChars(); });

  codeReset.addEventListener('click', () => {
    codeBlock.querySelectorAll('.code-str').forEach(el => {
  ...
  // Left offer cards sync with selector
  leftOffers.forEach(offer => {
    offer.addEventListener('click', () => {
      leftOffers.forEach(o => o.classList.remove('selected'));
      offer.classList.add('selected');
      const label = offer.querySelector('.offer-label').textContent.trim();
      offerBtns.forEach(b => {
        b.classList.remove('active');
        if (b.getAttribute('data-offer') === label) b.classList.add('active');
      });
    });
  });

  // Send button - opens Discord
  contactSend.addEventListener('click', () => {
    const msg = contactMessage.value.trim();
    const activeOffer = document.querySelector('.offer-btn.active');
    const offerName = activeOffer ? activeOffer.getAttribute('data-offer') : 'VIP';
    // Open Discord ticket channel
    window.open('https://discord.gg/leak2ids', '_blank');
    contactMessage.value = '';
    contactSend.textContent = '✓ Redirigé vers Discord !';
    setTimeout(() => {
      contactSend.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" width="16" height="16"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"/></svg> Ouvrir un ticket';
    }, 2500);
  });
});
